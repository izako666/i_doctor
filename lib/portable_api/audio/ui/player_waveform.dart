...
import 'package:i_doctor/portable_api/audio/audio_controller.dart';
import 'package:i_doctor/portable_api/audio/state_management/player_waveform_controller.dart';
...
class _PlayerWaveformState extends State<PlayerWaveform>
  ...
  void initState() {
    ...
    finalDbList = averageChunks(widget.dbList,
        (widget.dbList.length / (widget.isReply ? 66 : 22)).round());
    super.initState();
  }
  ...
  List<double> averageChunks(List<double> inputList, int chunkSize) {
    ...
    for (int i = 0; i < inputList.length; i += chunkSize) {
      int end =
          (i + chunkSize < inputList.length) ? i + chunkSize : inputList.length;
  ...
  List<double> scaleToMinimumZero(List<double> inputList) {
    ...
    List<double> scaledList =
        inputList.map((value) => value - minValue).toList();
